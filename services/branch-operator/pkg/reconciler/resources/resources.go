...
func PoolerSpec(clusterName string, instances int32, hibernated bool, poolMode apiv1.PgBouncerPoolMode, maxClientConn, defaultPoolSize string, podLabels map[string]string, imagePullSecrets []string, tolerations []v1.Toleration, nodeSelector map[string]string) apiv1.PoolerSpec {
	if hibernated {
		instances = 0
	}
